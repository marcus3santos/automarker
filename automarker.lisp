...
(defun automark (submissions-dir is-zipped grades-export-file test-cases-dir results due-date-time &optional (weights nil))
  (check-input-files (list submissions-dir grades-export-file test-cases-dir))
  (let* ((root-folder (check-foldername  (namestring (ensure-directories-exist results :verbose T))))
    ...
    (with-open-file (log-file-stream (merge-pathnames root-folder "log.txt") ; (merge-pathnames (asdf:system-source-directory :automarker) "log.txt")
                                     :direction :output
                                     :if-exists :append
